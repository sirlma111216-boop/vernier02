// /functions/api/evaluate.js — Cloudflare Pages Function
// 기본 제공사: Gemini(무료 티어). env.AI_PROVIDER = 'gemini' | 'claude' 로 교체 가능.
//
// ⚠️ 개인정보: 클라이언트는 학생 이름·학교·반을 여기로 보내지 않는다.
//    payload에는 익명화된 측정 데이터와 답변 텍스트만 담는다.
//    { dataSummary, predictions, observeAnswers, explainAnswers }
//
// 안정성/지역:
//   - 모든 오류 경로를 잡아 200 + {error, detail, ...flags} JSON으로 돌려준다.
//     (5xx로 돌려주면 Cloudflare 엣지가 "error code: 502" 평문으로 덮어써서
//      클라이언트가 원인을 알 수 없게 되므로, 본문이 항상 닿도록 200으로 반환.)
//   - 일부 엣지 colo(예: 홍콩 HKG)는 Gemini 미지원 지역이라 400 "User location is
//     not supported"를 반환. 이는 같은 invocation에서 해결 불가 → retryable=false.
//   - 일시적 오류(429/500/502/503/504)만 서버에서 최대 3회 재시도한다.
//   - 진단용으로 응답 헤더에 실행 colo와 Gemini 상태를 임시 노출(x-debug-*).

const SYSTEM_PROMPT = `너는 중학교 3학년 과학 '상태 변화와 열에너지' 단원의 탐구를 돕는 평가 도우미다.
학생은 라우르산(로르산)의 냉각 곡선 실험을 했다. 로르산의 응고점은 약 43°C다.
다음 원칙을 지켜라:
- 답을 곧장 주기보다, 오개념이 있으면 학생이 스스로 깨닫도록 되묻는 질문/힌트를 먼저 제시한다.
- 그래도 마지막에는 올바른 개념을 짧게 확인시켜 준다.
- 중학생 눈높이의 따뜻하고 구체적인 말투를 쓴다.
핵심 개념: 액체→고체로 응고하는 동안 열에너지를 '방출'하기 때문에 온도가 일정하게 유지된다. 냉각하면 입자 운동이 둔해지고 입자 사이 거리가 가까워지며 배열이 규칙적으로 변한다.

채점 기준(중요 — 학생 답을 이 정답과 정확히 매칭해 평가하라. 각 문항을 혼동하지 말 것):
- '관찰1_온도가_빠르게_내려가는_기울어진_구간의_상태'의 정답은 '액체'다. (아직 응고 전, 액체가 식는 구간)
- '관찰2_온도가_일정한_평평한_구간의_상태'의 정답은 '액체 + 고체'다. (응고가 진행되어 액체와 고체가 공존)
- '관찰3'(읽은 응고점)은 약 43°C 부근이면 맞다.
학생의 답이 위 정답과 일치하면 절대로 오개념으로 지적하지 말고 '잘된 점'으로 인정하라. 일치하지 않을 때만 misconceptions에 담아라.`;

const JSON_SCHEMA_HINT = `반드시 아래 JSON 스키마로만, 다른 텍스트 없이 응답하라:
{"success": boolean, "successReason": string, "strengths": [string], "improvements": [string], "misconceptions": [{"detected": string, "guidingQuestion": string, "correctConcept": string}], "encouragement": string}`;

const JSON_HEADERS = { 'content-type': 'application/json; charset=utf-8' };
const RETRY_STATUS = new Set([429, 500, 502, 503, 504]); // 이 상태만 재시도
const jsonResp = (obj, extra = {}) => new Response(JSON.stringify(obj), { headers: { ...JSON_HEADERS, ...extra } });
const sleep = ms => new Promise(r => setTimeout(r, ms));

async function fetchWithTimeout(url, opts, ms) {
  const ctrl = new AbortController();
  const id = setTimeout(() => ctrl.abort(), ms);
  try { return await fetch(url, { ...opts, signal: ctrl.signal }); }
  finally { clearTimeout(id); }
}

export async function onRequestPost({ request, env }) {
  const colo = (request.cf && request.cf.colo) || 'unknown'; // 실행된 엣지 데이터센터
  try {
    let payload;
    try { payload = await request.json(); }
    catch { payload = {}; }

    const provider = env.AI_PROVIDER || 'gemini';
    const userContent = `다음은 학생의 실험 데이터 요약과 답변이다. 평가해줘.\n${JSON.stringify(payload, null, 2)}`;

    const resultText = provider === 'claude'
      ? await callClaude(env, userContent)
      : await callGemini(env, userContent);

    const clean = (resultText || '').replace(/```json|```/g, '').trim();
    try { JSON.parse(clean); }
    catch { return jsonResp({ error: 'AI 응답 형식 오류', detail: clean.slice(0, 200) }, { 'x-debug-colo': colo, 'x-debug-gemini-status': '200-nonjson' }); }
    return new Response(clean, { headers: { ...JSON_HEADERS, 'x-debug-colo': colo, 'x-debug-gemini-status': '200' } });
  } catch (e) {
    const msg = String((e && e.message) || e);
    const statusMatch = msg.match(/Gemini (\d{3})/);
    const gStatus = statusMatch ? statusMatch[1] : 'n/a';
    // 지역 제한(미지원 colo)은 재시도 불가
    const geoBlocked = /location is not supported/i.test(msg) || (gStatus === '400' && /location/i.test(msg));
    const rateLimited = gStatus === '429';
    return jsonResp({
      error: geoBlocked ? '지역 제한(이 엣지에서는 Gemini 호출 불가)'
           : rateLimited ? 'AI 사용량 한도 초과(잠시 후 재시도)'
           : 'AI 평가 호출 실패',
      detail: msg,
      geoBlocked,
      rateLimited,
      retryable: !geoBlocked && (rateLimited || RETRY_STATUS.has(Number(gStatus))),
    }, { 'x-debug-colo': colo, 'x-debug-gemini-status': gStatus });
  }
}

// ── Gemini (기본) ───────────────────────────────────────────────
async function callGemini(env, userContent) {
  const model = env.GEMINI_MODEL || 'gemini-3.1-flash-lite';
  const geminiPayload = {
    systemInstruction: { parts: [{ text: `${SYSTEM_PROMPT}\n${JSON_SCHEMA_HINT}` }] },
    contents: [{ role: 'user', parts: [{ text: userContent }] }],
    generationConfig: {
      responseMimeType: 'application/json', // JSON 강제
      maxOutputTokens: 1500,
      temperature: 0.4,
    },
  };

  // GEMINI_PROXY_URL 이 설정되면 도쿄 Cloud Run 릴레이를 경유한다(지역 차단 회피).
  // 릴레이는 Gemini 응답을 상태/본문 그대로 돌려주므로 아래 처리 로직은 동일.
  let url, headers, body;
  if (env.GEMINI_PROXY_URL) {
    url = env.GEMINI_PROXY_URL;
    headers = { 'content-type': 'application/json', 'x-proxy-secret': env.PROXY_SHARED_SECRET || '' };
    body = JSON.stringify({ model, payload: geminiPayload });
  } else {
    if (!env.GEMINI_API_KEY) throw new Error('GEMINI_API_KEY 미설정');
    url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${env.GEMINI_API_KEY}`;
    headers = { 'content-type': 'application/json' };
    body = JSON.stringify(geminiPayload);
  }

  // 일시적 오류(429/5xx)만 백오프 후 재시도(최대 3회). 지역제한(400) 등은 즉시 중단.
  const delays = [0, 800, 2000, 4500];
  let lastErr;
  for (let i = 0; i < delays.length; i++) {
    if (delays[i]) await sleep(delays[i]);
    let res;
    try { res = await fetchWithTimeout(url, { method: 'POST', headers, body }, 25000); }
    catch (e) { lastErr = new Error('Gemini fetch 실패: ' + String((e && e.message) || e)); continue; } // 네트워크 오류는 재시도
    if (res.ok) {
      const data = await res.json();
      return (data.candidates?.[0]?.content?.parts || []).map(p => p.text || '').join('');
    }
    lastErr = new Error('Gemini ' + res.status + ' ' + (await res.text()).slice(0, 200));
    if (!RETRY_STATUS.has(res.status)) break; // 400(지역제한 포함) 등은 재시도 무의미
  }
  throw lastErr;
}

// ── Claude (교체용) ─────────────────────────────────────────────
async function callClaude(env, userContent) {
  if (!env.ANTHROPIC_API_KEY) throw new Error('ANTHROPIC_API_KEY 미설정');
  const res = await fetchWithTimeout('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'x-api-key': env.ANTHROPIC_API_KEY,
      'anthropic-version': '2023-06-01',
      'content-type': 'application/json',
    },
    body: JSON.stringify({
      model: env.CLAUDE_MODEL || 'claude-haiku-4-5',
      max_tokens: 1500,
      system: `${SYSTEM_PROMPT}\n${JSON_SCHEMA_HINT}`,
      messages: [{ role: 'user', content: userContent }],
    }),
  }, 25000);
  if (!res.ok) throw new Error('Claude ' + res.status + ' ' + (await res.text()).slice(0, 200));
  const data = await res.json();
  return (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('');
}
